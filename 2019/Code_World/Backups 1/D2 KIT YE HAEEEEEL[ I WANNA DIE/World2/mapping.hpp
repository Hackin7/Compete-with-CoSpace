...
/////////////////////////////////////////////////////////////////
int unexploredcol=-1;
int unexfloorcol = location.empty;
...
void overwrite(int x,int y, int color)
{
...
void colormap(int x, int y)
{
  ...
  if (colorCheck(w2Obj.objRed,10)){rectfill(redcol,findmax(PositionX-30,0),findmin(PositionX+30,360),findmax(PositionY-20,0),findmin(PositionY+20,270));}
  else if (colorCheck(w2Obj.objBlue,10)){rectfill(cyancol,findmax(PositionX-30,0),findmin(PositionX+30,360),findmax(PositionY-20,0),findmin(PositionY+20,270));}
  else if (colorCheck(w2Obj.objBlack,10)){rectfill(blackcol,findmax(PositionX-30,0),findmin(PositionX+30,360),findmax(PositionY-20,0),findmin(PositionY+20,270));}
...
void areafill(int x, int y, int val){
	int xval = 30, yval = 20;
	rectfill(val,findmax(x-xval,0),findmin(x+xval,360),findmax(y-yval,0),findmin(y+yval,270));
}
...
void laserempty(int us, double uscomp){
	...
	for (int k=0; k<abs(dx); k++){
		...
		//j2 = j+1;
		location.map[i][j] = unexfloorcol;
		squarefill(unexfloorcol, i, j, 1); // new
	}
	}
}
